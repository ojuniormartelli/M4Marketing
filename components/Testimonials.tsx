      ...
      <div className="container mx-auto px-4 md:px-8 fade-in-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((item, index) => (
            <div key={index} className="bg-white text-gray-800 rounded-2xl p-8 relative hover:shadow-lg transition-shadow">
              <div className="flex gap-1 mb-4 text-accent">
                {[...Array(5)].map((_, i) => <Star key={i} size={18} fill="currentColor" />)}
              </div>
              <p className="text-gray-600 mb-6 italic min-h-[80px]">"{item.text}"</p>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 font-semibold">{item.company}</p>
                </div>
              </div>
            </div>
          ))}
